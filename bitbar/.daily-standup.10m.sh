...
user=$(whoami)
username="Florian Mientjes"
PROJECTS_DIRECTORY="/Users/$user/projects/"
DAYS_SINCE_COMMIT=0

# Start
COUNT=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup | grep -c "<$username>")
COUNT="$COUNT"
if [ "$COUNT" = 0 ]
then
    COUNT=""
    ...
    DAYS_SINCE_COMMIT=1
    COUNT2=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup -d 2 | grep -c "<$username>")
    COUNT3=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup -d 3 | grep -c "<$username>")
    COUNT4=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup -d 4 | grep -c "<$username>")
    COUNT5=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup -d 5 | grep -c "<$username>")
    COUNT6=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup -d 6 | grep -c "<$username>")
    COUNT7=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup -d 7 | grep -c "<$username>")
    COUNT8=$(cd "$PROJECTS_DIRECTORY" || exit ;/usr/local/bin/git-standup -d 8 | grep -c "<$username>")
    if [ "$COUNT2" = 0 ]
    then
      COUNT=""
